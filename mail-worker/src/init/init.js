...
const dbInit = {
	...
	async v1_6DB(c) {
		...
		const noticeContent = 'This project is for learning and communication purposes only.\n' +
			'<br>\n' +
			'Please comply with local regulations. The author assumes no legal responsibility.'
		...
		const ADD_COLUMN_SQL_LIST = [
			...
			`ALTER TABLE setting ADD COLUMN notice_title TEXT NOT NULL DEFAULT 'CloudMail';`,
	...
	async intDB(c) {
		...
		try {
			await c.env.db.prepare(`
			  ...
			  SELECT 0, 0, 0, 0, 'CloudMail', 0, 1, 1
